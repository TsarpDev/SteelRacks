...
proc_ref	zic_beam2D_truss	{ Region_ID  beams  trusses  massX   *lengths_sym  h0_start  h0_end  x0  y0  {Node_ID 1000} {Ele_ID 1000}  {InternalNode_ID 1000  }  } {

	kill_if_3D "ERROR: zic_beam2D_truss is only for 2D analysis. Change analysis type.";
...
#         Generate Horizontal Beams			#
	...
	for	{set j 1} {$j <= [expr 2*($N_sym-1)] } {incr j} { 
		...
		element elasticBeamColumn	$current_ele  	$internal_nodeI  $internal_nodeJ   $A_h $Es $I_h $Ln_beam;
...
#         Generate Vertical Bracing         #
	...
	for	{set j 1} {$j <= [expr $total_nodes-1] } {incr j 2} {
		...
		element elasticBeamColumn	$current_ele  	$internal_nodeI  $internal_nodeJ   $A_v $Es $I_v $Ln_beam;
...
#         Generate Diagonal Bracing         #
	...
	for	{set j 2} {$j <= [expr $total_nodes-2] } {incr j 2} {
		...
		if {$j >= [expr $total_nodes/2]} {
			...
			element elasticBeamColumn	$current_ele  	$internal_nodeI  $internal_nodeJ   $A_d $Es $I_d $Ln_beam;
