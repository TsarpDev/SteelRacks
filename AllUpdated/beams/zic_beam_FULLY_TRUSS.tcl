proc_ref	zic_beam_FULLY_TRUSS	{ Region_ID  beams  trusses  massX   *lengths_sym  {h0_start 1.0} {h0_end 1.1}  {x0 0.0}  {y0 0.0}  {Node_ID 1000} {Ele_ID 1000}  {InternalNode_ID 1000  }  } {
...
#         Generate Horizontal Beams			#
	...
	for	{set j 1} {$j <= [expr 2*($N_sym-1)] } {incr j} { 
		...
		element truss $current_ele $node_i  $node_j $A_h 210 
...
#         Generate Diagonal Bracing         #
	...
	for	{set j 2} {$j <= [expr $total_nodes-2] } {incr j 2} {
		...
		if {$j >= [expr $total_nodes/2]} {
			...
			element truss $current_ele $node_i  $node_j $A_d 210 
