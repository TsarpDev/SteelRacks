#################################################################
#################### x_column2D_NL_FULLY_TRUSS ####################
#################################################################
#	Arbitrary x_column						  				    # 
#	uprights: LINEAR E-B						     			#
#	diagonals: NL truss						    				#
#	horizontals: LINEAR truss								    #
#	fixity: pinned											    #


proc_ref	x_column2D_NL_FULLY_TRUSS	{ Region_ID   *masses *heights  x0  h0 *columns *beams *bracings  release   {big_x 0} {imp 200} {Node_ID 1000} {Ele_ID 1000}  {InternalNode_ID 1000  } {nIntPt 3} {maxIters 20} {tolIter 1.e-8} } {


	kill_if_3D "ERROR: x_column2D_NL_FULLY_TRUSS is only for 2D analysis. Change analysis type.";
...
#         Generate Beam Bracing             #
	...
	for	{set j 1} {$j < $N_col} {incr j} {
		...
		for {set i 1} {$i < $N_box} {incr i} {
			...
			incr current_ele;			
			element truss $current_ele $node_i  $node_j $A_beam $sectionTag 
			write_element $current_ele $node_i  $node_j;
