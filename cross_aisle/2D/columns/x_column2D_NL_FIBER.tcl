...
#	uprights: NL FIBER							     			#
#	diagonals: NL FIBER					   		 				#
#	horizontals: NL FIBER									    #
#	fixity: depends 										    #




proc_ref	x_column2D_NL_FIBER	{ Region_ID   *masses *heights  x0  h0 *columns *beams *bracings  release   {big_x 0} {imp 200} {Node_ID 1000} {Ele_ID 1000}  {InternalNode_ID 1000  } {nIntPt 3} {maxIters 20} {tolIter 1.e-8} } {


		kill_if_3D "ERROR: x_column2D_NL_FIBER is only for 2D analysis. Change analysis type.";
...
#
		...
		for {set j 1} {$j <= $N_col} {incr j} {
			...
			if { [lindex $masses $i] > 0 } {
				if { [expr $j%2] == 0 && $N_col == 3} {
					...
					write_node_with_mass2D [expr $RegionNode_ID+$current_node] [expr 2*$mass_temp] $Negligible $Negligible;
				} else {
					node	[expr $RegionNode_ID+$current_node]		[expr $x0+$h0*($j-1)]		$H_temp 	-mass $mass_temp $Negligible $Negligible;
					write_node_with_mass2D [expr $RegionNode_ID+$current_node] $mass_temp $Negligible $Negligible;
				}
			} else {
	...
	addExternalNodes [expr $Region_ID*$Node_ID+1]	[expr $RegionNode_ID+$current_node-1];
...
#         Generate Columns					#
	...
	for	{set j 1} {$j <= $N_col} {incr j} {
		...
		for {set i 1} {$i <= $N_box} {incr i} {
			...
			element nonlinearBeamColumn   $current_ele  	$node_i  $node_j $nIntPt  $sectionTag  $Pd_col -iter $maxIters $tolIter
	...
	if { $big_x == 0 } {
		for	{set j 1} {$j < $N_col} {incr j} {
			...
			for {set i 1} {$i < $N_box} {incr i} {		# now it starts from second box
				...
				set internal_nodeJ [expr $node_j*$InternalNode_ID+1];
				set x_j [expr [nodeCoord $node_j 1]];
				set y_j [expr [nodeCoord $node_j 2]];
				node	[expr $internal_nodeJ]		$x_j	$y_j
				
				
				set imp_nod [generate2DImperfectionPoint [expr $RegionNode_ID+$current_node] $internal_nodeI $internal_nodeJ $imp]
				incr current_node
				
				#element nonlinearBeamColumn   $current_ele  	$internal_nodeI  $internal_nodeJ $nIntPt  $sectionTag  2 -iter $maxIters $tolIter
				incr current_ele;
				element nonlinearBeamColumn   $current_ele  	$internal_nodeI  $imp_nod $nIntPt  $sectionTag  $Crt_br -iter $maxIters $tolIter
				write_element $current_ele $node_i $imp_nod;
				
				
				incr current_ele;
				element nonlinearBeamColumn   $current_ele  	$imp_nod  $internal_nodeJ $nIntPt  $sectionTag  $Crt_br -iter $maxIters $tolIter
				...
				set internal_nodeI [expr $node_i*$InternalNode_ID+5];
				set x_i [expr [nodeCoord $node_i 1]];
				set y_i [expr [nodeCoord $node_i 2]];
				node	[expr $internal_nodeI]		$x_i	$y_i
