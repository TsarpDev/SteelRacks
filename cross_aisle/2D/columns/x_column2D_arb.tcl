...
#################### x_column2D_arb				 ################
#################################################################
#	Arbitrary LINEAR x_column				 				    # 
#	uprights: LINEAR E-B						     			#
#	diagonals: LINEAR E-B					    				#
#	horizontals: LINEAR E-B									    #
#	fixity: depends											    #



proc_ref	x_column2D_arb	{ Region_ID   *masses *heights  x0  h0 *columns *beams *bracings   release   {big_x 0} {Node_ID 1000} {Ele_ID 1000}  {InternalNode_ID 1000  } } {


	kill_if_3D "ERROR: x_column2D_arb is only for 2D analysis. Change analysis type.";
...
#         Generate Columns					#
	...
	for	{set j 1} {$j <= $N_col} {incr j} {
		...
		for {set i 1} {$i <= $N_box} {incr i} {
			...
			set A_col [get2DElasticA $sectionTag]
			set Es [get2DElasticE $sectionTag]
			set I_col [get2DElasticIz $sectionTag]
			
			
			
			element elasticBeamColumn	$current_ele  	$node_i  $node_j  $A_col $Es $I_col $Pd_col;
	...
	if { $big_x == 0 } {
		for	{set j 1} {$j < $N_col} {incr j} {
			...
			for {set i 1} {$i < $N_box} {incr i} {		# now it starts from second box
				...
				set Es [get2DElasticE $sectionTag]
				set I_br [get2DElasticIz $sectionTag]
				...
				element elasticBeamColumn	$current_ele  	$internal_nodeI  $internal_nodeJ  $A_br $Es $I_br $Crt_br;
		...
		for	{set j 1} {$j < $N_col} {incr j} {
			...
			for {set i 3} {$i <= $N_box} {incr i} {
				...
				set A_br [get2DElasticA $sectionTag]
				set Es [get2DElasticE $sectionTag]
				set I_br [getElasticIz $sectionTag]
...
#         Generate Beam Bracing             #
	...
	for	{set j 1} {$j < $N_col} {incr j} {
		...
		for {set i 1} {$i < $N_box} {incr i} {
			...
			set Es [get2DElasticE $sectionTag]
			set I_beam [get2DElasticIz $sectionTag]
			...
			element elasticBeamColumn	$current_ele  	$internal_nodeI  $internal_nodeJ  $A_beam $Es $I_beam $Ln_beam;
