...
if {$analysisType == "compression"} {
	...
	for {set i [expr [llength $strain]-1]} {$i >= 0} {set i [expr $i-1]} {
...
proc combine {length} {
	...
	fforeach a "fittedC.txt" {
		lappend dc [expr -[lindex $a 0]/$length]
		lappend Nc [expr -[lindex $a 1]]
	}
	
	set Nt {}
	set dt {}
	fforeach a "fittedT.txt" {
		lappend dt [expr [lindex $a 0]/$length]
		lappend Nt [lindex $a 1]
	}
	...
	set fileID_write [open "output.txt" "w"];
	
	for {set i 0} {$i<[llength $N]} {incr i} {
		puts   $fileID_write "[lindex $d $i] [lindex $N $i]";
	}
